...
/**
 * Helper para extrair e validar obra_ativa_id do usuário autenticado
 * Retorna a obra ativa do usuário ou lança erro
 */
export async function getObraAtivaFromUser(
  req: Request,
  res: Response
): Promise<string | null> {
  const usuarioId = (req as any).user?.id;
  ...
  // Busca o usuário com obra_ativa_id
  const usuario = await prisma.usuario.findUnique({
    where: { id: usuarioId },
  ...
  if (!usuario.obra_ativa_id) {
    res.status(400).json({
      error: 'Obra ativa não definida',
      message: 'Usuário não possui obra ativa definida. Defina uma obra ativa antes de acessar este endpoint.',
    });
    return null;
  }

  // Valida se a obra existe e não foi deletada
  const obra = await prisma.obra.findUnique({
  ...
  if (!obra) {
    res.status(404).json({
      error: 'Obra ativa não encontrada',
      message: `Obra ativa com ID ${usuario.obra_ativa_id} não existe`,
    });
    return null;
  }
  ...
  if (obra.deleted_at) {
    res.status(404).json({
      error: 'Obra ativa deletada',
      message: `Obra ativa com ID ${usuario.obra_ativa_id} foi deletada`,
    });
    return null;
  }

  return usuario.obra_ativa_id;
}
